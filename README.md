...
`tools/list` at `https://gateway.pipeworx.io/nih-reporter/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_grants \
  -H 'Content-Type: application/json' \
  -d '{"query":"cancer immunotherapy","ic":"NCI","fiscal_year":2024,"limit":25}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_grants`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
